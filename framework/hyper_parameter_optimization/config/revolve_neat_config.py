...
class RevolveNeatConfig(NeatGenomeConfigGenericMixin):
    ...
    conn_add_prob: float = 0.08
    conn_delete_prob: float = 0.02
    enabled_default: bool = True
    enabled_mutate_rate: float = 0.01
    feed_forward: bool = True
    initial_connection: str = "full_direct"
    node_add_prob: float = 0.01
    node_delete_prob: float = 0.0
    num_hidden: int = 1
    num_inputs: int = 4
    num_outputs: int = 1
    response_init_mean: float = 1.0
    response_init_stdev: float = 0.0
    response_max_value: float = 10.0
    response_min_value: float = -10.0
