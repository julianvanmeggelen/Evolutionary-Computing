...
def main(config: RevolveNeatConfig, plot=True, callback=None, save_winner = False, verbose=1) -> None:
    ...
    initial_fitnesses = [
        evaluate(genotype, config) for genotype in initial_genotypes
    ]
    ...
    while generation_index < config.NUM_GENERATIONS:
        ...
        offspring_fitnesses = [
            evaluate(genotype, config) for genotype in offspring_genotypes
        ]
...
if __name__ == "__main__":
    config = RevolveNeatConfig() #default config
    main(config)