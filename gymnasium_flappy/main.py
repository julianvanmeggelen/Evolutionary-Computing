...
def main(plot=True, config_in=None, callback=None, save_winner = False) -> None:
    ...
    while generation_index < config.NUM_GENERATIONS:
        ...
        if curr_max > all_time_best_fitness:
            all_time_best_fitness = curr_max
            all_time_best_genotype = max(population, key = lambda ind: ind.fitness)
            print('saving')
            with open('winner-feedforward_overall', 'wb') as f:
                pickle.dump(all_time_best_genotype, f)
        ...
        #offspr_max, offspr_mean, offspr_min = [fun([_.fitness for _ in offspring_population]) for fun in (np.max,np.mean,np.min)]
        #logging.info(f"Offspring: {offspr_max:.2f}, {offspr_mean:.2f}, {offspr_min:.2f}")
